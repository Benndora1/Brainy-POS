...
class Waiter_model extends CI_Model {
    ...
    public function getSaleBySaleId($sales_id){
        $this->db->select("tbl_sales.*,tbl_users.full_name as waiter_name,tbl_customers.name as customer_name,tbl_tables.name as table_name,tbl_users.full_name as user_name,tbl_outlets.invoice_footer as invoice_footer");
        $this->db->from('tbl_sales');
        $this->db->join('tbl_customers', 'tbl_customers.id = tbl_sales.customer_id', 'left');
