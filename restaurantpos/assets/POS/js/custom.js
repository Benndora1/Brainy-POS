...
$(document).ready(function () {
	...
	$('.bottom_add').on('click', function () {
        ...
        var now_available = parseInt(sit_capacity_number) - (Number(person)+total_person);
		...
		table_book_row += '<div class="floatleft fix column third_column person_tbl_'+table_id+'">' + person + '</div>';
	...
	$(document).on('click', '.remove_new_order_row_icon', function () {
		...
		$(this).parent().parent().remove();
        var person = $('#single_table_order_details_bottom_person_' + this_table_id).val();
	...
	$('.overlayForCalculator').on('click', function () {
		...
		$('#calculator_main').fadeToggle(333);
		$('.overlayForCalculator').slideUp(111);
	});
	...
	$("#search").on('keyup', function (e) {
		...
		var searched_category_items_to_show = '<div id="searched_item_found" class="specific_category_items_holder" style="display:flex;">';
	...
	$('#dine_in_button,#take_away_button,#delivery_button').on('click', function () {
		...
		if ($(this).attr('id') == 'dine_in_button') {} else if ($(this).attr('id') == 'take_away_button') {
				...
				for (var key in table_details) {
					...
					if (orders_table_number > 0) {
						for (var key2 in orders_table) {
							...
							tables_modal += '<div class="floatleft fix column third_column person_tbl_'+table_id+'">' + orders_table[key2].persons + '</div>';
	...
	$('#place_order_operation').on('click', function () {
		...
		if (selected_order_type_object.length > 0) {
			...
			if (selected_order_type_object.attr('id') == 'delivery_button') {
				order_type = 3;
				if (customer_id == "") {
					$("#select2-walk_in_customer-container").css("border","1px solid red");
					/*swal({
						title: warning + "!",
			...
			if ($('.order_holder .single_order').length > 0) {
				...
				$('.order_holder .single_order').each(function (i, obj) {
					...
					items_info += '"item_unit_price":"' + item_unit_price + '","item_quantity":"' + item_quantity + '","tmp_qty":"' + tmp_qty + '","p_qty":"' + p_qty + '",';
...
$('#print_kot_modal').on('click', function () {
    ...
    if (order_details_id != "") {
        ...
        order_details_id_array.forEach(function (entry) {
            ...
            items_info += '{"kot_item_name":"' + kot_item_name + '", "kot_item_qty":"' + kot_item_qty + '", "tmp_qty":"' + tmp_qty + '", "p_qty":"' + p_qty + '"';
            var modifiers = '';
            var m = 1;
            ...
            for (var key in response.items) {
                ...
                if (modifier_length > 0) {
                    for (var index = 0; index < modifier_length; index++) {
                        modifier_divs += (selected_modifiers_array[index] != "") ? '<span>'+selected_modifiers_array[index]+'</span>, ' : '';
                    }
                }
                var discount_value = (this_item.menu_discount_value != "") ? this_item.menu_discount_value : '0.00';
                if(this_item.item_type!="Bar Item"){
                    ...
                    draw_table_for_kot_modal += '<input type="hidden" value="'+this_item.menu_note+'" id="kot_modal_note_'+this_item.id+'"><div style="width: 405px" class="kot_content_column fix floatleft kot_item_name_column">';
                    draw_table_for_kot_modal += this_item.menu_name;
                    draw_table_for_kot_modal += modifier_divs?'<br>Modifiers: '+modifier_divs:'';
                    draw_table_for_kot_modal += this_item.menu_note?'<br>Notes: '+this_item.menu_note:'';
                    draw_table_for_kot_modal += '</div>';
                    ...
                    draw_table_for_kot_modal += ' <span style="display:none" id="kot_modal_item_qty_fixed_' + this_item.id + '">' + this_item.qty + '</span><span id="kot_modal_item_qty_' + this_item.id + '">' + this_item.qty + '</span> <input type="hidden" name="tmp_qty" value="' + this_item.tmp_qty + '" id="tmp_qty_' + this_item.id + '"><input type="hidden" name="p_qty" value="' + this_item.qty + '" id="p_qty_' + this_item.id + '">';
...
function print_bot(sale_id) {
    let newWindow = open("print_bot/" + sale_id, 'Print BOT', 'width=450,height=550')
    newWindow.focus();
...
$('#print_bot_modal').on('click', function () {
    ...
    var order_details_id = '';
    var j = 1;
    var checkbox_length = $('.single_bot_row .bot_check_column .bot_item_checkbox:checked').length;
    $('.single_bot_row .bot_check_column .bot_item_checkbox:checked').each(function (i, obj) {
        if (j == checkbox_length) {
            order_details_id += $(this).val();
        } else {
            order_details_id += $(this).val() + ',';
    ...
    if (order_details_id != "") {
        ...
        order_details_id_array.forEach(function (entry) {
            ...
            items_info += '{"bot_item_name":"' + bot_item_name + '", "bot_item_qty":"' + bot_item_qty + '", "tmp_qty":"' + tmp_qty + '", "p_qty":"' + p_qty + '"';
            var modifiers = '';
            var m = 1;
            $('.single_modifier:visible').each(function (i, obj) {
            	var this_id = $(this).attr("data-item-detail-id");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response > 0) {
                ...
                let newWindow = open("print_temp_bot/" + response, 'Print BOT', 'width=450,height=550')
                newWindow.focus();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var key in response.items) {
                ...
                var discount_value = (this_item.menu_discount_value != "") ? this_item.menu_discount_value : '0.00';
                if(this_item.item_type=="Bar Item"){
                    draw_table_for_bot_modal += '<div class="single_bot_row fix" id="bot_for_item_' + this_item.id + '">';
                    draw_table_for_bot_modal += '<div class="bot_content_column fix floatleft bot_check_column">';
                    draw_table_for_bot_modal += '<input checked class="bot_item_checkbox" id="bot_item_checkbox_' + this_item.id + '" type="checkbox" name="" value="' + this_item.id + '">';
                    draw_table_for_bot_modal += '</div>';
                    draw_table_for_bot_modal += '<input type="hidden" value="'+this_item.menu_note+'" id="bot_modal_note_'+this_item.id+'"> <div style="width: 405px" class="bot_content_column fix floatleft bot_item_name_column">';
                    draw_table_for_bot_modal += this_item.menu_name;
                    draw_table_for_bot_modal += modifier_divs?'<br>Modifiers: '+modifier_divs:'';
                    draw_table_for_bot_modal += this_item.menu_note?'<br>Notes: '+this_item.menu_note:'';
                    draw_table_for_bot_modal += '</div>';
                    draw_table_for_bot_modal += '<div class="bot_content_column fix floatleft bot_qty_column">';
                    draw_table_for_bot_modal += '<i style="cursor:pointer;" id="decrease_item_bot_modal_' + this_item.id + '" class="fas fa-minus-circle decrease_item_bot_modal"></i>';
                    draw_table_for_bot_modal += ' <span style="display:none" id="bot_modal_item_qty_fixed_' + this_item.id + '">' + this_item.qty + '</span><span id="bot_modal_item_qty_' + this_item.id + '">' + this_item.qty + '</span> <input type="hidden" name="tmp_qty" value="' + this_item.tmp_qty + '" id="tmp_qty_' + this_item.id + '"><input type="hidden" name="p_qty" value="' + this_item.qty + '" id="p_qty_' + this_item.id + '">';
                    draw_table_for_bot_modal += '<i style="cursor:pointer;" id="increase_item_bot_modal_' + this_item.id + '" class="fas fa-plus-circle increase_item_bot_modal"></i>';
                    draw_table_for_bot_modal += '</div>';
                    draw_table_for_bot_modal += '</div>';
                }
                // draw_table_for_order_details += '<div class="single_item_modifier fix" id="order_details_for_item_'+this_item.food_menu_id+'">';
	...
	$.ajax({
		...
		success: function (response) {
			...
			for (var key in response) {
				...
				$order_list_left += '<span id="open_orders_order_status_' + response[key].sales_id + '" style="display:none;">' + response[key].order_status + '</span><p  class="oder_list_class" style="font-size: 16px;text-align:left;width: 125px;float: left;">Order: <span class="running_order_order_number">' + order_name + '</span></p><img src="' + base_url + 'assets/images/right-arrow.png" style="float: right;width: 13px;margin: 2px;transition: .25s ease-out;" class="running_order_right_arrow" id="running_order_right_arrow_' + response[key].sales_id + '">';
	...
	$.ajax({
		...
		success: function (response) {
			...
			for (var key in response) {
				...
				$order_list_left += '<span id="open_orders_order_status_' + response[key].sales_id + '" style="display:none;">' + response[key].order_status + '</span><p   class="oder_list_class" style="font-size: 16px;font-weight: bold;text-align:left;width:125px;float:left">Order: <span class="running_order_order_number">' + order_name + '</span></p><img src="' + base_url + 'assets/images/right-arrow.png" style="float: right;width: 13px;margin: 2px;transition: .25s ease-out;" class="running_order_right_arrow" id="running_order_right_arrow_' + response[key].sales_id + '">';
				$order_list_left += '<p>Table: <span class="running_order_table_name">' + tables_booked + '</span></p>';
				$order_list_left += '<p>Waiter: <span class="running_order_waiter_name">' + waiter_name + '</span></p>';
				$order_list_left += '<p>Customer: <span class="running_order_customer_name">' + customer_name + '</span></p>';
				$order_list_left += '</div>';
...
function arrange_info_on_the_cart_to_modify(response) {
	...
	for (var key in response.items) {
		...
		draw_table_for_order += '<div class="single_order_column first_column fix"><i style="cursor:pointer;" class="fas fa-pencil-alt edit_item" id="edit_item_' + this_item.food_menu_id + '"></i> <span id="item_name_table_' + this_item.food_menu_id + '">' + this_item.menu_name + '</span></div>';
		draw_table_for_order += '<div class="single_order_column second_column fix">' + currency + ' <span id="item_price_table_' + this_item.food_menu_id + '">' + this_item.menu_unit_price + '</span></div>';
		draw_table_for_order += '<div class="single_order_column third_column fix"><input type="hidden" name="tmp_qty" value="' + this_item.tmp_qty + '" id="tmp_qty_' + this_item.food_menu_id + '"> <input type="hidden" name="p_qty" value="' + this_item.qty + '" id="p_qty_' + this_item.food_menu_id + '"> <i style="cursor:pointer;" class="fas fa-minus-circle decrease_item_table" id="decrease_item_table_' + this_item.food_menu_id + '"></i> <span id="item_quantity_table_' + this_item.food_menu_id + '">' + this_item.qty + '</span> <i style="cursor:pointer;" class="fas fa-plus-circle increase_item_table" id="increase_item_table_' + this_item.food_menu_id + '"></i></div>';
		draw_table_for_order += '<div class="single_order_column forth_column fix"><input type="" name="" placeholder="Amt or %" class="special_textbox" id="percentage_table_' + this_item.food_menu_id + '" value="' + this_item.menu_discount_value + '" disabled></div>';
		draw_table_for_order += '<div class="single_order_column fifth_column fix">' + currency + ' <span id="item_total_price_table_' + this_item.food_menu_id + '">' + this_item.menu_price_with_discount + '</span></div>';
		draw_table_for_order += '</div>';
		if (selected_modifiers != "") {
...
function createAnimation(sale_id) {
    setTimeout(function(){
        $(".order_details").animate({scrollTop:(Number($(".order_details").height())+2000)},2000);
